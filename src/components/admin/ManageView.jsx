    ...
    <main className="space-y-6 animate-in slide-in-from-right duration-500 font-cyber pb-24 relative">
        ...
        <div className="fixed inset-0 z-[99999] flex flex-col justify-end p-2 sm:p-4">
          ...
          <div className="bg-card border-t-2 sm:border-2 border-primary w-full max-w-lg mx-auto rounded-t-3xl sm:rounded-3xl shadow-2xl dark:shadow-[0_-10px_40px_rgba(var(--primary),0.2)] relative z-10 overflow-hidden animate-in slide-in-from-bottom-full duration-300 flex flex-col max-h-[85vh]">
            ...
            <div className="bg-input/50 dark:bg-black/50 p-4 border-b border-border flex justify-between items-center shrink-0">
              ...
              <button onClick={() => setIsCatalogOpen(false)} className="text-muted hover:text-main dark:hover:text-white p-1 hover:bg-input rounded transition-colors"><X size={24} /></button>
            </div>
            ...
            <div className="p-4 shrink-0 border-b border-border bg-input/20">
              <div className="relative">
                ...
                <input type="text" placeholder="Buscar exercício..." className="w-full bg-input border-2 border-border p-3 pl-10 rounded-xl outline-none focus:border-primary text-sm text-main dark:text-white font-bold uppercase transition-all placeholder-muted/50 shadow-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
            </div>
            ...
            <div className="overflow-y-auto p-4 space-y-2 flex-1 min-h-[40vh]">
              {getFilteredExercises().map((ex, idx) => {
                ...
                return (
                  <div key={idx} onClick={() => toggleSelection(ex)} className={`flex items-center justify-between p-4 rounded-xl cursor-pointer transition-all border-2 ${isSelected ? 'border-primary bg-primary/10 shadow-sm dark:shadow-[0_0_10px_rgba(var(--primary),0.2)]' : 'border-border bg-card hover:border-primary/50'}`}>
                    <span className={`font-bold text-xs uppercase ${isSelected ? 'text-primary' : 'text-main dark:text-white'}`}>{ex}</span>
      ...
      {isAddDayModalOpen && createPortal(
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
          ...
          <div className="bg-card border-2 border-primary w-full max-w-sm p-6 rounded-2xl shadow-2xl dark:shadow-[0_0_30px_rgba(var(--primary),0.3)] relative z-10 animate-in zoom-in-95 duration-200">

...
const MuscleHeatmap = ({ history }) => {
  ...
  // 🔥 Funções de Estilo Tático Otimizadas para Claro/Escuro
  const getHeatColor = (sets, target = 12) => {
    const percentage = Math.min(100, (sets / target) * 100);
    // Cor inativa agora usa transparência de preto/branco baseada no CSS para não manchar o modo claro
    if (percentage === 0) return 'rgba(128, 128, 128, 0.15)'; 
    if (percentage < 50) return 'rgba(var(--primary), 0.4)'; 
    if (percentage < 100) return 'rgba(var(--primary), 0.8)'; 
    return 'rgba(239, 68, 68, 0.9)'; // Tailwind red-500
  };
  ...
  return (
    <div className="bg-card border border-border p-5 rounded-3xl shadow-sm mt-6 relative overflow-hidden">
      {/* 🔥 Efeito de grid tático - Agora usa primary sutil para funcionar em ambos os temas */}
      <div className="absolute inset-0 bg-[linear-gradient(rgba(var(--primary),0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(var(--primary),0.05)_1px,transparent_1px)] bg-[size:20px_20px] pointer-events-none"></div>
      
      {/* Header com Seletor de Biotipo */}
      <div className="flex items-center justify-between mb-6 relative z-10">
        <h3 className="text-sm font-black text-primary uppercase tracking-widest flex items-center gap-2 drop-shadow-[0_0_5px_rgba(var(--primary),0.3)]">
          <Activity size={18} /> Scanner Biométrico <span className="text-[10px] text-main/50">(7D)</span>
        </h3>
        
        <div className="flex items-center gap-1 bg-input p-1 rounded-lg border border-border shadow-inner">
          <button 
            onClick={() => setGender('male')}
            className={`p-1.5 rounded-md transition-all ${gender === 'male' ? 'bg-primary text-white shadow-[0_0_10px_rgba(var(--primary),0.5)]' : 'text-main/40 hover:text-main'}`}
          >
            <User size={16} />
          </button>
          <button 
            onClick={() => setGender('female')}
            className={`p-1.5 rounded-md transition-all ${gender === 'female' ? 'bg-primary text-white shadow-[0_0_10px_rgba(var(--primary),0.5)]' : 'text-main/40 hover:text-main'}`}
          >
            <Users size={16} />
          </button>
        </div>
      ...
      <div className="flex justify-center items-center py-4 relative z-10">
        ...
        <svg viewBox="0 0 200 300" className="w-56 h-auto filter drop-shadow-[0_0_15px_rgba(var(--primary),0.15)]">
          <defs>
            <pattern id="scanlines" patternUnits="userSpaceOnUse" width="4" height="4">
              {/* Opacidade reduzida para ficar elegante no claro também */}
              <line x1="0" y1="0" x2="200" y2="0" stroke="rgba(128,128,128,0.2)" strokeWidth="2"/>
            </pattern>
          ...
          {gender === 'male' ? (
            <g className="male-wireframe">
               <polygon points="90,20 110,20 115,40 100,50 85,40" fill="rgba(128,128,128,0.2)" stroke="rgba(var(--primary),0.3)"/>
            ...
            <g className="female-wireframe">
              <polygon points="92,25 108,25 112,42 100,50 88,42" fill="rgba(128,128,128,0.2)" stroke="rgba(var(--primary),0.3)"/>
        ...
        {/* 🔥 LEGENDA FLUTUANTE HUD MELHORADA - Adaptada para Claro/Escuro */}
        <div className="absolute right-2 top-16 flex flex-col gap-2 text-[8px] font-mono uppercase tracking-widest text-main/70 text-right bg-card p-2 rounded-lg border border-border shadow-sm z-20">
          <div className="flex items-center gap-1 justify-end text-red-500 font-bold">
            <Activity size={10} className="animate-pulse" /> Sobrecarga
          </div>
          ...
          <div className="flex items-center gap-1 justify-end opacity-60">
            <span className="w-2 h-2 rounded-full bg-main/30"></span> Descanso
          </div>
        </div>
      </div>

      {/* 🔥 DADOS BRUTOS - Legibilidade Ouro 🔥 */}
      <div className="grid grid-cols-4 gap-1.5 mt-2 pt-4 border-t border-border relative z-10">
        {Object.entries(heatData).map(([muscle, sets]) => {
          const isOverload = sets >= 12;
          return (
          <div key={muscle} className={`text-center rounded-lg py-1 border shadow-inner ${isOverload ? 'bg-red-500/10 border-red-500/30' : 'bg-input border-transparent'}`}>
            <p className="text-[7px] font-black text-main/60 uppercase">{muscle}</p>
            <p className={`text-xs font-black leading-none mt-0.5 ${isOverload ? 'text-red-600 dark:text-red-500 drop-shadow-[0_0_5px_rgba(239,68,68,0.4)] animate-pulse' : 'text-primary drop-shadow-[0_0_2px_rgba(var(--primary),0.2)]'}`}>
              {sets}
            </p>
          </div>
        )})}
      ...
      <style jsx>{`
        .neon-pulse-blue { filter: drop-shadow(0 0 8px rgba(var(--primary), 0.5)); }
        .neon-pulse-red { animation: pulse-red 2s infinite; }
        @keyframes pulse-red {
          0% { filter: drop-shadow(0 0 5px rgba(239, 68, 68, 0.4)); fill: rgba(239, 68, 68, 0.6); }
          50% { filter: drop-shadow(0 0 15px rgba(239, 68, 68, 0.8)); fill: rgba(239, 68, 68, 0.9); }
          100% { filter: drop-shadow(0 0 5px rgba(239, 68, 68, 0.4)); fill: rgba(239, 68, 68, 0.6); }
        }
